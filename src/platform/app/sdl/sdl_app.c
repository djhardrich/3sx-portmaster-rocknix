...
static bool init_window() {
    ...
    if (mode != NULL) {
        ...
        SDL_Log("WARNING: SDL_GetCurrentDisplayMode failed (%s); "
                "falling back to %dx%d",
                SDL_GetError(), window_width, window_height);
    }
#else
...
#if CRS_PLATFORM_PORTMASTER
    // No mouse on these handhelds. Video subsystem is already up
    // (SDL_INIT_VIDEO happens in pre_init()) so this call is safe here.
    SDL_HideCursor();
#endif
...
static void update_metrics(Uint64 sleep_time) {
    ...
    static size_t filled_slots = 0;

    const size_t ring_capacity = SDL_arraysize(frame_metrics.fps);
    if (filled_slots < ring_capacity) {
        filled_slots += 1;
    }

    // Seed on first call so the first log fires ~5s after game start,
    // not ~5s after device boot.
    if (last_log_time == 0) {
    ...
    if (new_frame_end_time - last_log_time >= 5000000000ULL) {
        ...
        const float divisor = (float)filled_slots;
        SDL_Log("[perf] fps_avg=%.1f frame_ms_avg=%.2f idle_ms_avg=%.2f",
                fps_sum / divisor, ft_sum / divisor, idle_sum / divisor);
        last_log_time = new_frame_end_time;
    }
#endif
}
...
static int loop() {
    ...
    while (is_running) {
        switch (phase) {
        case APP_PHASE_INIT: {
            const int pre_init_status = pre_init();

#if CRS_PLATFORM_PORTMASTER
            // On the H700 there is no UI to recover from a failed SDL
            // init, so any non-zero status is fatal. Continuing would
            // segfault on a NULL renderer in the next phase.
            if (pre_init_status != 0) {
                SDL_Log("FATAL: pre_init failed; exiting.");
                is_running = false;
                break;
            }
#else
            (void)pre_init_status;
#endif

            if (Resources_Check()) {
                const int full_init_status = full_init();

#if CRS_PLATFORM_PORTMASTER
                if (full_init_status != 0) {
                    SDL_Log("FATAL: full_init failed; exiting.");
                    is_running = false;
                    break;
                }
#else
                (void)full_init_status;
#endif
